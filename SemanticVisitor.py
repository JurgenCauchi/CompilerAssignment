...
class TypeChecker(Visitor):
    ...
    def visit_ASTProgramNode(self, node):
        ...
        print(
            # Debug
            f"Type checking program with {len(node.statements)} statements")
    ...
    def visit_ASTAddOpNode(self, node):
        ...
        if left_type is None:
            self._add_error(
                f"Left operand has no type (missing visitor for {type(node.left).__name__}?)")
            return "error"
        if right_type is None:
            self._add_error(
                f"Right operand has no type (missing visitor for {type(node.right).__name__}?)")
            return "error"

        # Type checking
        if left_type != right_type:
            self._add_error(
                f"Type mismatch in operation: {left_type} {node.op} {right_type}")
            return "error"
    ...
    def visit_ASTWriteNode(self, node):
        ...
        if node.kw == 0 and node.size != 5:
            self._add_error(
                "Write Box must have exactly 5 arguments (x, y, width, height, colour)")
        elif node.kw == 1 and node.size != 3:
            self._add_error(
                "Write must have exactly 3 arguments (x, y, colour)")
